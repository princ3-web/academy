...
const ImageSlider = () => {
  ...
  return (
    <Box className={imageSliderStyles.slider} paddingX={4}>
      {images.map((item, n) => (
        <div className={imageSliderStyles.slide} style={{ filter: n === index ? "Opacity(1)" : "Opacity(0)" }}>
          <img className={imageSliderStyles.image} src={item.image} alt="" />
          <Box
            p={3}
            className={imageSliderStyles.textBox}
            sx={{ boxShadow: 10, width: 400, height: 100, backgroundColor: grey[100], borderRadius: 2 }}
          >
