...
const MultipleCarousel = ({ elements, index }) => {
  ...
  return (
    <Box sx={{ position: "relative", width: "100%" }}>
      ...
      <Box sx={{display:"flex", justifyContent:"space-between", position:"absolute", width: "110%", left: "-5%", bottom: "50%"}}>
        <IconButton
          onClick={handleScrollLeft}
          disabled={scrollAmount === 0}
        >
          <ArrowCircleLeft style={{ fontSize: "250%" }} sx={{ color: grey[900], opacity: 1 }} />
        </IconButton>
        <IconButton
          onClick={handleScrollRight}
          disabled={scrollAmount >= items.length * 210} // Adjust 210 based on item width + margin
        >
          <ArrowCircleRight style={{ fontSize: "250%" }} sx={{ color: grey[900], opacity: 1 }} />
        </IconButton>
      </Box>
    </Box>
  );
