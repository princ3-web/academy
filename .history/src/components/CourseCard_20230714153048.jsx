...
const CourseCard = ({ boxWidth, width, height, imageObjectFit, title, text, rating }) => {
  ...
  return (
    <Button sx={{ textTransform: "none", p: 0, m: 0 }}>
      ...
      >
        <Image width={width} height={height} objectFit={imageObjectFit} src={educationImage} />
        <Box height={150}>
          ...
          <Box display={"flex"}>
            <Typography color="darkorange" fontSize={16} fontWeight={600} mr={1}>
              {rating}
            </Typography>
            <StarRating key ={}feffefeqwweffw rating={rating} />
          </Box>
          <Typography fontSize={20} color={teal[600]} fontWeight={600}>
            99,99 zł
          </Typography>
        </Box>
        <Box>
