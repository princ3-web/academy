    ...
    <div className={imageSliderStyles.slider}>
      {images.map((item, n) => (
        <div className={imageSliderStyles.slide}>
          <img className={imageSliderStyles.image} style={{translate: 100% * index}} src={item.image} alt="" />
        </div>
      ))}
      <div className={imageSliderStyles.arrows}>
        <IconButton onClick={() => (index > 0 ? setIndex((state) => state - 1) : setIndex(images.length))}>
          <KeyboardArrowLeftIcon fontSize="large" style={{ color: grey[50] }} className={imageSliderStyles.arrow} />
        </IconButton>
        <IconButton onClick={() => (index < images.length - 1 ? setIndex((state) => state + 1) : setIndex(0))}>
