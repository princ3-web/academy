...
const ImageSlider = () => {
  ...
  return (
    <div className={imageSliderStyles.slider}>
      {images.map((object, n) => (
        <img className={imageSliderStyles.image} style={{ left: index === n ? "0%" : "100%" }} src={object} alt="" />
      ))}
      ...
      <div className={imageSliderStyles.arrows}>
        <img
          ...
          onClick={() => (index < images.length ? setIndex((state) => state + 1) : "")}
          style={{ rotate: "180deg" }}
          src={arrow}
          alt=""
        />
        <img
          className={imageSliderStyles.arrow}
          onClick={() => (index > 0 ? setIndex((state) => state - 1) : "")}
          src={arrow}
          alt=""
        />
      </div>
