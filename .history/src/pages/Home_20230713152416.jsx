  ...
  return (
    <Box>
      ...
      >
        ...
        <MultipleCarousel
          elements={testimonials.map((item, n) => (
            <TestimonialCard name={item.name} text={item.text} width={isMobile ? "75vw" : "25vw"} />
          ))}
          index={selectedItem}
        />
      </Box>
      ...
      >
        ...
        <Box display={"flex"} flexWrap={"wrap"}>
          {categories.map((item) => (
            <Box width={isMobile ? "50%" : "25%"} display={"flex"} flexDirection={"column"} mb={3}>
              <Typography variant={"h6"} fontWeight={600} mb={1} textAlign={isMobile ? "center" : "flex-start"}>
                {item.name}
              </Typography>
              {item.array.map((i) => (
                <Button
