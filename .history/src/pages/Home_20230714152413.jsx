  ...
  return (
    <Box>
      ...
      <Box position={"relative"} width={"90vw"} left={"5vw"} py={8}>
        ...
        <Box my={2}>
          <FormControl fullWidth>
            ...
            >
              {courses.map((item, n) => (
                <MenuItem key={item.name} value={n}>{item.name}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Box>
      ...
      >
        ...
        key={"MultipleCarousel1"}
          elements={testimonials.map((item, n) => (
            <TestimonialCard key={item.name} name={item.name} text={item.text} width={isMobile ? "75vw" : "25vw"} />
          ))}
          index={selectedItem}
        />
      </Box>
      ...
      >
        ...
        <Box display={"flex"} flexWrap={"wrap"}>
          {categories.map((item) => (
            <Box key={item.name} width={isMobile ? "50%" : "25%"} display={"flex"} flexDirection={"column"} mb={3}>
