            ...
            >
              <Button>
                <Box sx={{ backgroundColor: grey[200], justifyContent: "center" }} p={2}>
                  <Image src={item.icon} objectFit={"cover"} height={160} width={300} />
                </Box>
              </Button>
        ...
        <Box display={"flex"} flexWrap={"wrap"}>
          {categories.map((item) => (
            <Box width={"25%"} display={"flex"} flexDirection={"column"} mb={3}>
              ...
              {item.array.map((i) => (
                <Button sx={{textTransform:"none", display: "flex", flexDirection:"column", justifyContent:"flex-start"}}>
                  <Typography fontWeight={600} color={green[700]}>{i}</Typography>
                  <Typography fontSize={14} color={grey[600]} mb={2}>{0} students</Typography>
                </Button>
              ))}
            </Box>
          ))}
